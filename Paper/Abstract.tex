...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{Loss Function}: To maximize character sharpness, we utilized the \textbf{L1 Loss} (Mean Absolute Error) during training. Unlike L2 Loss (MSE), which tends to produce overly smooth textures, L1 Loss effectively preserves the sharp edges required for OCR.
		\item \textbf{Training Configuration}: The model was trained for \textbf{500 epochs} using the Adam optimizer with an initial learning rate of $2 \times 10^{-4}$ and a cosine annealing scheduler.
	\end{itemize}
	...
	\subsubsection{Implementation and Training Details}
	The restoration module was implemented using PyTorch. We utilized the \textbf{L1 Loss} function (Mean Absolute Error) during training, as it promotes sharper edge reconstruction compared to L2 Loss (MSE), which tends to produce overly smooth textures.
	
	The model was trained for \textbf{500 epochs} using the Adam optimizer with an initial learning rate of $\mathbf{2 \times 10^{-4}}$. We employed a learning rate scheduler to stabilize convergence. The training process was monitored using the Peak Signal-to-Noise Ratio (PSNR) metric on a held-out validation set \cite{conde2022swin2sr}. The final model checkpoint was selected based on the best validation performance, achieving a PSNR of \textbf{27.39 dB}, which indicates a significant recovery of image quality sufficient for OCR readability.
